...
import (
	...
	"Nezha/ethereum/go-ethereum/common/math"
...
func (l *StructLogger) CaptureState(env *EVM, pc uint64, op OpCode, gas, cost uint64, memory *Memory, stack *Stack, contract *Contract, depth int, err error) error {
	...
	// initialise new changed values and read values storage container for this contract
	// if not present.
	if l.changedValues[contract.Address()] == nil {
		l.changedValues[contract.Address()] = make(Storage)
	}
	if l.readValues[contract.Address()] == nil {
		l.readValues[contract.Address()] = make(Storage)
	}
	...
	if op == SLOAD && stack.len() >= 1 {
		...
		l.readValues[contract.Address()][address] = value
...
// RWSetCapture returns the changed values of addresses
func (l *StructLogger) RWSetCapture(addr common.Address) (Storage, Storage) {
	return l.readValues[addr], l.changedValues[addr]
}
